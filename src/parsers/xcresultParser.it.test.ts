...
describe("extractXcResultFile", () => {
    ...
    test.each([
        ...
        "test-results-retry.xcresult",
    ])("xcrun produces json [%p]", async (xcresultsDir) => {
        const jsonFile = extractXcResultFile(`samples/${xcresultsDir}`)
        const actualJson = jsonFile ? JSON.parse(await fs.promises.readFile(jsonFile, {encoding: "utf-8"})) : null;
