...
export async function resolveFile(filepath: string, ...possibleExtensions: string[]) {
    ...
    const paths =
        possibleExtensions.length > 0 ? possibleExtensions.map((ext) => `**/${filepath}.${ext}`) : [`**/${filepath}`];

    const file = await getFirstGitFile(paths);
    if (file) {
        const relativePath = relative(cwd(), file);
        core.debug(
            `File \`${relativePath}\` found for \`${filepath}\` with possible extensions ${possibleExtensions.join(", ")}`,
        );
    ...
    if (core.isDebug()) {
        core.warning(
            `Could not find a file matching \`${filepath}\` with possible extensions ${possibleExtensions.join(", ")}`,
        );
    }
    return filepath;
}
