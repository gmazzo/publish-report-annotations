...
describe("config", () => {
    test.each([
        ...
        "can read config [testsSummaryInput=%p, checksSummaryInput=%p]",
        async (testsSummaryInput, checksSummaryInput) => {
            ...
            if (testsSummaryInput == "<other>") {
                await expect(readConfig()).rejects.toThrow(
                    `Invalid value for 'testsSummary': '<other>', possible values are: 'full', 'suitesOnly', 'totals', 'off'`,
                );
                return;
            }
            if (checksSummaryInput == "<other>") {
                await expect(readConfig()).rejects.toThrow(
                    `Invalid value for 'checksSummary': '<other>', possible values are: 'full', 'totals', 'off'`,
                );
                return;
            }
    ...
    test.each([
        ...
        "computes workflowSummary correctly [workflowSummary=%p, checkName=%p, expectedWorkflowSummary=%p]",
        async (workflowSummary, checkName, expectedWorkflowSummary) => {
            getInput.mockImplementation((name: string) => {
                switch (name) {
                    case "workflowSummary":
