...
| `summary`          | The kind of summary to report into the check, multiples values are accepted:<ul><li>`testCases`<br/>Reports each individual test case execution</li><li>`testSuites`<br/>Reports each test suite (with a resume of its child test cases)<br/>When combined with `testCases`, its child test casses are grouped with a `<details>`</li><li>`skipPassed`<br/>Only reports test suites or cases that has not been passed (reducing verbosity)</li><li>`checks`<br/>Reports checks, one table per check type</li><li>`off`<br/>Does not prints any summary</li></ul><br/>⚠️ Keep in mind on large builds, it may reach [the GitHub's limit](https://docs.github.com/en/rest/checks/runs?apiVersion=2022-11-28#create-a-check-run-output-object) on large builds. Try reducing the level of details if that's the case (i.e. `testSuites` is less verbose than `testSuites` + `testCases` | `testsSuites`<br/>`checks`                                       |
